...
br_model_all = BayesianRidge(fit_intercept=false, 
                        alpha_1=1e-6, 
                        alpha_2=1e-6, 
                        lambda_1=1e-6, 
                        lambda_2=1e-6, 
                        tol=1e-6, 
                        max_iter=300)
br_model_all.fit(Awp["train"], Yw["train"])
Σ = pyconvert(Matrix, br_model_all.sigma_)
coef_tilde_all = pyconvert(Array, br_model_all.coef_)
alpha_all = pyconvert(Float64, br_model_all.alpha_) # estimate of noise precision
coef_all = Psqrt \ coef_tilde_all
ACEpotentials.Models.set_linear_parameters!(model, coef_all)
comp_potE_error(raw_data["train"], model)
comp_potE_error(raw_data["test"], model)
comp_potE_error(raw_data["val"], model)
