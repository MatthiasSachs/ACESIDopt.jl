...
using ACESIDopt.MSamplers: mala_step!, run_mala_sampling
...
# Run MALA sampling for glycine at 300 K
println("\n" * "="^60)
println("Starting Metropolis-Adjusted Langevin Algorithm sampling")
println("="^60)

mala_initial = deepcopy(raw_data["train"][1])
T_mala = 300.0  # K
n_mala_samples = 10000
step_size_mala = 0.001  # Å - MALA typically allows larger step sizes than RWMC
burnin_mala = 2000
thin_mala = 10
...
mala_samples, mala_acceptance, mala_energies = run_mala_sampling(
    mala_initial, model, n_mala_samples, T_mala;
    step_size=step_size_mala, burnin=burnin_mala, thin=thin_mala
)

# Plot MALA energy distribution
histogram(mala_energies,
         bins=50,
         xlabel="Potential Energy (eV)",
         ylabel="Frequency",
         title="MALA Sampled Energy Distribution at $(T_mala) K",
         legend=false,
         alpha=0.7)

# Plot energy trace to check for convergence
plot(1:length(mala_energies), mala_energies,
     xlabel="Sample Number",
     ylabel="Potential Energy (eV)",
     title="MALA Energy Trace",
     linewidth=1,
     legend=false)

#%%
mean(potential_energy(d, model) for d in mala_samples)
...
# Save MALA samples to extended XYZ file
ExtXYZ.save("./test-traj/mala_samples.xyz", [m for m in mala_samples])
