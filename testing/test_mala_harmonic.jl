"""
Test MALA sampling with HarmonicCalculator

This test applies Metropolis-Adjusted Langevin Algorithm (MALA) sampling to sample the 
...
MALA uses gradient information (forces) to guide proposals, making it more efficient
than random walk Monte Carlo for sampling from smooth distributions.

The test verifies that:
1. MALA converges to the correct distribution
...
using ACESIDopt.MSamplers: run_mala_sampling, MALASampler, run_sampler
...
println("="^60)
println("Testing MALA Sampling with HarmonicCalculator")
println("="^60)
...
# Use ExtXYZ.Atoms for mutability (required by MALA)
...
step_size = 0.15       # Step size for MALA (Å)

println("\n--- Running MALA Sampling ---")
println("MALA Parameters:")
println("  Samples to collect: $n_samples")
println("  Burn-in steps: $burnin")
println("  Thinning: $thin")
println("  Step size: $step_size Å")

# Run MALA sampling using the new unified interface
mala_sampler = MALASampler(step_size=step_size)
samples, acceptance_rate, traj = run_sampler(mala_sampler, system, calc, T; n_samples=n_samples, burnin=burnin, thin=thin, collect_forces=false)

# Alternative: use the old interface (still supported for backward compatibility)
# samples, acceptance_rate, traj = run_mala_sampling(
...
output_file = joinpath(output_dir, "mala_harmonic_test.png")
savefig(combined_plot, output_file)
println("Saved diagnostic plots to: $output_file")

# Summary
println("\n" * "="^60)
println("MALA Sampling Test Summary")
println("="^60)
...
if acceptance_rate > 0.2 && acceptance_rate < 0.7 &&
   ...
   drift < 0.05
    ...
    println("MALA sampling successfully samples the Gibbs-Boltzmann distribution")
else
