...
# This test demonstrates using the ACE potential trained on Si with RWMC (Random Walk Metropolis-Hastings)
...
println("="^70)
println("Testing RWMC Sampling with ACE Model for Silicon")
println("="^70)
...
step_size = 0.01      # Step size for RWMC (Å) - can be larger than MALA
...
println("\n" * "="^70)
println("Running RWMC Sampling with ACE Model")
println("="^70)
println("Note: This may take several minutes due to the expensive ACE energy calculations...")
println("Timing RWMC sampling...")

# Run RWMC sampling with timing
...
if 0.15 < acceptance_rate < 0.4
    println("  $(GREEN)✓ Good$(RESET) - Optimal range for RWMC is ~0.234")
elseif 0.1 < acceptance_rate < 0.5
...
p1 = plot(1:length(energies), energies ./ 64,
         ...
         title="RWMC Energy Trajectory (T=$T K)",
         label="",
         linewidth=1,
         alpha=0.7,
         color=:blue,
...
output_file = joinpath(output_dir, "rwmc_ace_silicon_test.png")
savefig(combined_plot, output_file)
println("Saved diagnostic plots to: $output_file")

# --- Summary ---
println("\n" * "="^70)
println("Summary: RWMC Sampling with ACE Model for Silicon")
println("="^70)
println("$(GREEN)✓$(RESET) RWMC sampling with ACE potential completed successfully")
...
println("\nPhysical Insights:")
println("  - RWMC uses random walk proposals (no gradient information)")
println("  - Simpler than MALA but may be less efficient")
...
println("\nAlgorithm Details:")
println("  - Random Walk Metropolis-Hastings (RWMC)")
println("  - Uses uniform random proposals without gradient information")
println("  - Optimal acceptance rate: ~23.4% (for high-dimensional problems)")
